...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a nuovocoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no nuovocoin: URI
...
{
    // Convert nuovocoin:// to nuovocoin:
    //
    //    Cannot handle this later, because nuovocoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open nuovocoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "nuovocoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for nuovocoin.lnk
...
{
    return GetAutostartDir() / "nuovocoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a nuovocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=nuovocoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the nuovocoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add nuovocoin app to startup item list
