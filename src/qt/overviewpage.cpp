...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizenuovocoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizenuovocoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizenuovocoinAmount = strAnonymizenuovocoinAmount.remove(strAnonymizenuovocoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizenuovocoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizenuovocoinAmount * COIN) nMaxToAnonymize = nAnonymizenuovocoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizenuovocoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizenuovocoinAmount));
        strAnonymizenuovocoinAmount = strAnonymizenuovocoinAmount.remove(strAnonymizenuovocoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizenuovocoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizenuovocoinAmount == 0) {
